...
const svgUp = `<svg class='filterSelect-select-svg' width="12" height="9" viewBox="0 0 12 9" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M6 0L11.1962 9L0.803847 9L6 0Z" fill="#F7F8FA"/>
</svg>
`;
...
class Data {
  constructor({ rawdata }) {
    ...
    this._data = this.roll_up(
      this._rawdata,
      (d) => d.brand,
      (d) => d.device_cummulative_cnts
    );
  }
...
class Bubble {
  ...
  _makeGradient() {
    ...
    lg.append("stop").attr("offset", "0%").attr("stop-color", "#DB6EA3");

    lg.append("stop").attr("offset", "100%").attr("stop-color", "#8884FF");
  }
  ...
  _makePack() {
    return (data) =>
      d3
        ...
        .padding(1)(
        d3.hierarchy({ children: this._data.data }).sum((d) => d.value)
      );
  }
  ...
  _textUpdate(update) {
    return update.attr("font-size", "0px").call((update) =>
      update
        .transition(this._t)
        .delay(500)
  ...
  _makeBubbles() {
    ...
    this.leaf = this.svg
      ...
      .join(
        ...
        (update) =>
          update.attr("transform", (d) => `translate(${d.x + 1},${d.y + 1})`),
        (exit) =>
          exit.call((exit) =>
            exit
              .transition(d3.transition().duration(1150))
              .attr(
                "transform",
                (d) =>
                  `translate(${this._getRandomRingCoords(3000)[0] + 1},${
                    this._getRandomRingCoords(3000)[1] + 1
                  })`
              )
              .remove()
          )
    ...
    this.leaf
      ...
      .join(
        (enter) => this._textEnterTitle(enter),
        (update) => this._textUpdate(update),
        (exit) => this._textExit(exit)
      );

    this.leaf
      .selectAll(".circle-text-text-perc")
      .data((d) => d)
      .join("text")
      ...
      .join(
        (enter) => this._textEnterPerc(enter),
  ...
  _setOnHover() {
    function mouseover() {
      ...
      d3.selectAll(".circle-text-text").transition(1000).attr("opacity", 0.2);

      d3.select(this).transition(1000).attr("fill-opacity", 1);
      d3.select(this.parentNode)
        .selectAll(".circle-text-text")
        .transition(1000)
        .attr("opacity", 1);
      ...
      d3.select(this.parentNode)
        ...
        .transition(t_)
        .attr("font-size", "28px")
        .text((d) => (d.text != "0.00%" ? d.text : "<0.01%"));
      d3.select(this.parentNode)
    ...
    function mouseout() {
      const t_ = d3.transition(1000);
      d3.selectAll(".circle-element").transition(1000).attr("fill-opacity", 1);

      d3.selectAll(".circle-text-text").transition(1000).attr("opacity", 1);
...
class Selector {
  ...
  makeInnerDiv() {
    this._mainSelection = this.mainDiv
      .selectAll("div")
      .data(this._divElData)
      .join("div")
  ...
  makeSelectionButton() {
    ...
    this._filter_selection = d3
      .select(".filterSelect")
      .append("div")
      .attr("class", "filterSelect-select-button")
      .attr("value", this.selection)
      .html(
        `<div class='filterSelect-select-div' >${svgDown} <span>${text} </span> </div>`
      );

    this.selectionHover();
  }

  makeSelectionButtonUp() {
    const self = this;
    d3.select(".filterSelect-select-button").remove();
    const text = this.selection === "All" ? "Select category" : this.selection;
    this._filter_selection = d3
      ...
      .attr("value", this.selection)
      .html(
        `<div class='filterSelect-select-div' >${svgUp} <span>${text} </span> </div>`
      );
  ...
  _onChange() {
    const self = this;
    const selection = d3
      .selectAll(".selection-menu-custom-divs")
...
class Menu {
  ...
  _makeMenuData() {
    ...
    return [
      {
        title: "Brands",
        value: this._brand_showing
          ? this._data.brandName
          : d3.format(",")(this._data.distinctBrands),
        style: this._brand_showing ? { "font-size": "35px" } : {},
      },
      {
        title: "Models",
        value: d3.format(",")(this._data.devicetypeValue),
        style: this._brand_showing ? { "font-size": "35px" } : {},
      },
    ];
  }
...
class Back {
  ...
  render() {
    d3.select(this._el)
      .attr("class", "d3-bubble-chart-selector-title title-active")
      .html(`<div>${svgBack} <span>${"Back"} </span> </div>`);
    this._onClick();
  }
  unrender() {
    d3.select(this._el)
      .attr("class", "d3-bubble-chart-selector-title")
...
class ChartFull {
  ...
  setOnClick() {
    ...
    const button = d3.select(this._el).append("button").text("Reset Chart");
